 ...
 */
define("local_booking/logbook_std",["jquery","core/str","core/pending","core/modal_factory","core/templates","core/notification","local_booking/booking_view_manager","local_booking/modal_actions","local_booking/events","local_booking/modal_logentry_form","local_booking/repository","local_booking/selectors"],(function($,Str,Pending,ModalFactory,Templates,Notification,ViewManager,ModalActions,BookingEvents,ModalLogentryEditForm,Repository,Selectors){const refreshLogbookContent=(root,logentry)=>{let card=$("#cardid_"+logentry.id);showPlaceholder(card);const courseId=$(Selectors.logbookwrapper).data("courseid"),userId=$(Selectors.logbookwrapper).data("userid");return M.util.js_pending(root.get("id")+"-"+courseId),Repository.getLogentryById(logentry.id,courseId,userId).then((context=>Templates.render("local_booking/logbook_std_detail",context.logentry))).then(((html,js)=>Templates.replaceNode(card,html,js))).always((()=>(M.util.js_complete(root.get("id")+"-"+courseId),showContent(card)))).fail(Notification.exception)},registerLogentryEditForm=(root,e,contextId,courseId,userId,logentryId)=>{const LogentryFormPromise=ModalFactory.create({type:ModalLogentryEditForm.TYPE,large:!0}),target=e.target,pendingPromise=new Pending("local_booking/registerLogentryEditForm");ViewManager.renderLogentryModal(root,e,LogentryFormPromise,target,contextId,courseId,userId,logentryId,!1,"local_booking/logbook_std").then(pendingPromise.resolve()).catch(window.console.error)},showPlaceholder=function(card){card.find(Selectors.containers.loadingPlaceholder).removeClass("hidden"),card.find(Selectors.containers.content).addClass("hidden")},showContent=function(card){card.find(Selectors.containers.content).removeClass("hidden"),card.find(Selectors.containers.loadingPlaceholder).addClass("hidden")};return{init:function(rt){var root=$(rt);(root=>{$("body").on(BookingEvents.logentryupdated,(function(e,logentry){refreshLogbookContent(root,logentry)})),$("body").on(BookingEvents.logentrydeleted,(function(e,logentryid){$("#logentry_"+logentryid).slideUp(300),e.stopImmediatePropagation()}));const contextId=$(Selectors.logbookwrapper).data("contextid"),courseId=$(Selectors.logbookwrapper).data("courseid"),userId=$(Selectors.logbookwrapper).data("userid");contextId&&root.on("click",Selectors.actions.edit,(function(e){let logentryId=e.target.closest(Selectors.containers.summaryForm).dataset.logentryId;e.preventDefault(),e.stopPropagation(),registerLogentryEditForm(root,e,contextId,courseId,userId,logentryId),e.stopImmediatePropagation()}))})(root),ModalActions.registerDelete(root),ViewManager.stopLoading(root)}}}));
