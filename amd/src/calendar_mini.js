 ...
 *
 * @module     local_booking/calendar_mini
...
function(
    ...
    var registerCalendarEventListeners = function(root) {
        ...
        body.on(CalendarEvents.created + namespace, root, reloadWeek);
        body.on(CalendarEvents.deleted + namespace, root, reloadWeek);
        body.on(CalendarEvents.updated + namespace, root, reloadWeek);
        body.on(CalendarEvents.eventMoved + namespace, root, reloadWeek);
    };

    /**
     * Reload the week view in this week.
     *
     * @param {EventFacade} e
     */
    var reloadWeek = function(e) {
        var root = e.data;
        var body = $('body');
        var namespace = '.' + root.attr('id');

        if (root.is(':visible')) {
            CalendarViewManager.reloadCurrentWeek(root);
        } else {
    ...
    var registerEventListeners = function(root) {
        ...
        $('body').on('change' + namespace, CalendarSelectors.elements.courseSelector, function() {
            if (root.is(':visible')) {
                ...
                CalendarViewManager.reloadCurrentWeek(root, courseId, categoryId);
            } else {
